...
def filling_mindee_xml(image_file, file_path_):
  ...
  for item in myroot[16].iter('{http://isdoc.cz/namespace/2013}Name'):
    ...
    myfile.write('Supplier company DIC ;'+ item.text)
    myfile.writelines('\n')
...
# define upload_function used in upload_button
def upload_file(file):
  UPLOAD_FOLDER = "./data13"
  if not os.path.exists (UPLOAD_FOLDER):
    oS.mkdir (UPLOAD_FOLDER)
    shutil.copy (file, UPLOAD_FOLDER)
  qr. Info ("File Uploaded Now Query the Document")
...
with gr.Blocks( theme=gr.themes.Soft() ) as demo:
    ...
    gr.Markdown(
    """
    ## CONVERT INVOICE IMAGES TO *.ISDOC FORMAT

    This is the first version that also process item table.
    """)
    ...
    #file_save=gr.Button("File save")
    #file_save.click(fn=convert, inputs=gr.FileExplorer(), outputs=output, api_name="convert")


    #filepath = working_dir + "/isdoc"
    #file_save_btn = gr.Button("Here", link=filepath)
...
#demo = gr.Interface(fn=convert, inputs=gr.Files(label='Choose files to be uploaded'), outputs="textbox",
#                    title="                 Conversion faktura.image  to isdoc format",
#    description="This is the first  version that converts the complete  faktura.image to isdoc format and validate",
#                    css=".gradio-container{background-color: green}")
#
